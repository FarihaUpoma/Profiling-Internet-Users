...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 16:27:21 2019
...
def populate_weekSheet():
    ...
    for k in range(len(df['Real First Packet'])):
        ...
        if delta.days >= 0 and delta.days < 5 and xx.time() >= startTime.time() and xx.time() < endTime.time() and df['Duration'][k] > 0:
            ...
            indx = delta.days * 3240 + (hour-stHour) * 360 + minute * 6 + int((second/10))
        ...
        #week2 calculation
            ...
            indx = delta2.days * 3240 + (hour-stHour) * 360 + minute * 6 + int((second/10))
            od = df['doctets'][k] / df['Duration'][k]
            if countVal[indx] == 0:
...
#for user in range(35,36):
    ...
    x = time.strftime("%A %d %m %Y %H %M %S +0000", time.localtime(t))
    x = x.split(" ")
    print("working with", excelFiles[user])
    adjust = 0
    if x[0] != 'Monday':
        for i in range(5):
            if weekdays[i] == x[0]:
    ...
    #print(filename)
    #filename1 = "Subject1_Week1.xlsx"
    #filename2 = "Subject1_Week2.xlsx"
...
#            continue
        ...
        if math.isnan(r1a2a) or math.isnan(r1a2b) or math.isnan(r2a2b):
            pVal = 0
        else:
...
os.chdir("../")
finalTable.to_excel("results/finalTable10.xlsx")
